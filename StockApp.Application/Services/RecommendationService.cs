...
namespace StockApp.Application.Services
{
    public class RecommendationService : IRecommendationService
    {
        ...
        public async Task<IEnumerable<Product>>GetRecommendationsAsync(string userId)
        {
            ...
            var recommendedProducts = userOrders.SelectMany(order => order.Products).GroupBy(product => product.Id).OrderByDescending(group => group.Count()).Select(group  => group.First()).Take(5);
